...
app.get("/logindoctor",(req,res)=>{
    res.render("doctor",{message: req.flash('message') })
})
...
app.post("/logindoctor",(req,res)=>{
    ...
    db.Admin.findOne({ email: email }, function (err, info) {
        if (err) {
            ...
            res.redirect("http://127.0.0.1:8080/patient.html")
        } else {
...
app.get("/python",(req,res)=>{
    var first = 5 ;
    var process = spawn('python',["./object_detection.py"]) ;
...
// app.get("/higherResolve",(req,res)=>{
//     res.render("higherResolve");
// })
